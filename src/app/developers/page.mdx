...
## 7. Penarikan dana (withdrawals) & settlement
...
1. Dapatkan API key (portal bila punya akses developer, atau minta admin).
2. Kirim `X-API-Key` di setiap request ke `…/api/v2/…`.
3. Buat pembayaran → tampilkan instruksi ke pembeli → tunggu callback / poll status.
4. Tarik saldo via `/withdrawals`.
5. Referensi lengkap & coba langsung: [`/api/v2/docs`](https://api.internal-go.saranatechnology.com/api/v2/docs).
